...
inT2 <- read.table("T2_plant_phenotyping.txt",header=T,sep="\t") 
inT2.2 <- read.table("T2_plant_phenotyping.notebook.txt",header=T,sep="\t") 
...
phenoT2.2 <- inT2 %>% select(Genotype,Phenotype) %>% rbind(inT2.2) %>% group_by(Genotype,Phenotype) %>% 
  ...
  filter(Genotype != "MK016") %>% filter(Tg == "35S:RUBY") %>% mutate(Genotype = case_when(Genotype == "MK063" ~ "MK016", TRUE ~ Genotype)) %>% 
  mutate(Genotype = str_replace(Genotype,"MK0",""))
...
library(patchwork)
library(ggh4x)
setwd("/Users/mariannekramer/Google Drive/Kramer_et_al_AIO/Figures/ruby_qpcr/")

plate1_rnd1 <- read_excel("220623_ruby_target_capture_valid_plate1.xlsx",sheet="Results",range="D52:O315",col_names =c("Sample","Target","task","reporter","quencher","quantity","quant mean","quant sd","rq","rq min","rq max","CT"))
plate2_rnd1 <- read_excel("220623_ruby_target_capture_valid_plate2.xlsx",sheet="Results",range="D52:O63",col_names =c("Sample","Target","task","reporter","quencher","quantity","quant mean","quant sd","rq","rq min","rq max","CT"))

plate1_rnd1 <- plate1_rnd1 %>%
  select(c(Sample,Target,CT)) %>% filter(Sample != 'NA'& Sample != "water") %>%
...
#plate1 %>% filter(`Sample Name` == "water")

input_rnd1 <- data_rnd1 %>% 
  filter(Target == "AT1G08200" | Target == "UBC10") %>%
  group_by(Sample) %>%
  dplyr::summarize(avg_ctrl_CT = mean(CT))
  
analysis_rnd1 <- data_rnd1 %>% 
  right_join(input_rnd1, by = c("Sample")) %>%
  mutate(dCt = CT-avg_ctrl_CT) %>%
  mutate(RQ = 2**(-dCt))

toPlot_rnd1 <- analysis_rnd1 %>%
  filter(Target != "AT1G08200" & Target != "UBC10") %>% 
  mutate(Sample = case_when(Sample == "02_MK017_Green_Rep1" ~ "02_MK017_8green_Rep1", TRUE ~ Sample)) %>%
  arrange(Sample) %>% separate(Sample, into = c("Number","Genotype","Pheno","Rep"), sep="_") %>%
  unite(c("Genotype","Rep"), col="Sample",sep = "_") %>%
  group_by(Number,Sample,Pheno,Target) %>%
  dplyr::summarize(RQ=mean(RQ)) %>% mutate(Pheno = case_when(Pheno == "Red" ~ "0Red",
                                                             Pheno == "FullRed" ~"1Fully Red",
                                                             Pheno == "RedParts" ~"2Red Parts",
                                                             Pheno == "FullGreen" ~"4Fully Green",
                                                             Pheno == "GreenParts" ~"3Green Parts", 
                                                             TRUE ~ Pheno)) %>%
  filter(Sample != "MK001_Rep1" & !grepl("MK034",Sample)& !grepl("MK010",Sample)& !grepl("MK040",Sample)) %>% 
  separate(Sample,into=c("geno","rep")) %>% 
  mutate(rep = str_replace(rep,"Rep","Rep "),
...
ggplot(toPlot_rnd1,aes(x=Pheno,y=RQ,fill=Pheno)) + geom_bar(stat="identity",position=position_dodge(0.9),color='black')+
  ...
  ggtitle("Abundance of RUBY relative to housekeeping genes")
...
library(patchwork)
library(ggpubr)
setwd("/Users/mariannekramer/Google Drive/Kramer_et_al_AIO/Figures/ruby_bsas/")
...
data <- mutate(data, Per=ifelse(Per == 100.0, 50, Per))
...
data <- mutate(data, Per=ifelse(strand == "minus", -Per, Per))
...
a<-data %>% separate(col=amplicon, sep = "MCK",into=c("amplicon", "sampleNum")) %>% 
  mutate(sampleNum = as.integer(sampleNum),
         target = case_when(grepl("RUBY",amplicon) ~ "RUBY", TRUE ~ "TyrAT"),
         amplicon = case_when(amplicon == "RUBY_A_" ~ "35S Promoter",amplicon == "RUBY_B_"~"CDS",amplicon == "TyrAT_"~"AT2G20680", TRUE ~ amplicon))


b <- a %>% left_join(sampleInfo,by="sampleNum") %>% separate(info, into=c("geno","prom","pheno"),sep="[.]") %>% 
...
ggplot(b, aes(fill=context, y=Per, x=pheno)) + 
  ...
  ggtitle("DNA methylation in CG, CHG, and CHH Contexts")
