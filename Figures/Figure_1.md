...
phenoT2.2 <- inT2 %>% select(Genotype,Phenotype) %>% rbind(inT2.2) %>% group_by(Genotype,Phenotype) %>% 
  ...
  dplyr::filter(Genotype != "MK016") %>% dplyr::filter(Tg == "35S:RUBY") %>% dplyr::mutate(Genotype = case_when(Genotype == "MK063" ~ "MK016", TRUE ~ Genotype)) %>% 
...
![](Figure_1_files/figure-gfm/unnamed-chunk-1-1.png)<!-- -->
...
toPlot_rnd1 <- analysis_rnd1 %>%
  ...
  dplyr::summarize(RQ=mean(RQ)) %>% dplyr::mutate(Pheno = case_when(Pheno == "Red" ~ "0Red",
...
```
    ...
    ## `summarise()` has grouped output by 'Number', 'Sample', 'Pheno'. You can
...
ggplot(b, aes(fill=context, y=Per, x=pheno)) + 
  ...
  ggtitle("DNA methylation in CG, CHG, and CHH")
```

![](Figure_1_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->
