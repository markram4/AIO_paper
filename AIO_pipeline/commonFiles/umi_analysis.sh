...
#SBATCH --output=/cluster/pixstor/slotkinr-lab/mkramer/projects/target_capture/logs/umi_%A_%a.txt
#SBATCH --error=/cluster/pixstor/slotkinr-lab/mkramer/projects/target_capture/logs/umi_%A_%a.err
...
UMIDIR="$BASEDIR/umi"
STRANDDIR="$BASEDIR/strandedness"
...
for file in "$STRANDDIR"/*sense.bed; do
    ...
    while IFS= read -r target; do
        ...
        grep "$target" "$file" | cut -f4 | sort -u > "$output_file"
    done < "$TARGETS"
done
...
while IFS= read -r line; do echo -e "Gene\tCount\tSample" > "$CLUSTERDIR"/$line".mapped_to_targets.umiCount.txt"; done < $SAMPLES 
...
for dir in $CLUSTERDIR/*; do
    if [ -d "$dir" ]; then
        ...
        for file in $dir/*fasta; do
            ...
            ## Get out file
            outFile="$CLUSTERDIR/$genotype.mapped_to_targets.umiCount.txt"
...
echo -e "Gene\tCount\tSample" > $CLUSTERDIR/"all.combined_numReads_mapped_to_targets.strandedness.perpA.$PROJECT.umi.txt"
for file in $CLUSTERDIR/*.umiCount.txt; do tail -n +2 $file >> $CLUSTERDIR/"all.combined_numReads_mapped_to_targets.strandedness.perpA.$PROJECT.umi.txt"; done
